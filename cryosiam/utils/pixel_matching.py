# Code taken from: https://github.com/teamtomo/membrain-seg/blob/main/src/membrain_seg/tomo_preprocessing/matching_utils/px_matching_utils.py
...
def match_pixel_size(
    ...
    ----------
    input_tomogram : np.array
        Input tomogram to be processed.
    pixel_size_in : float
    ...
    output_shape = determine_output_shape(
        pixel_size_in, pixel_size_out, input_tomogram.shape
    )

    # Perform Fourier-based resizing (cropping or extending) using the determined
    # output shape
    if (pixel_size_in / pixel_size_out) < 1.0:
        resized_data = fourier_cropping(input_tomogram, output_shape, smoothing)
    elif (pixel_size_in / pixel_size_out) > 1.0:
        resized_data = fourier_extend(input_tomogram, output_shape, smoothing)
    else:
        resized_data = input_tomogram
